...
export class KanbanService {
  ...
  public getPartyByRefAndClient(party_type: string, client_ref: string){
    return this.partyByTypeGQL.watch({party_type, client_ref}).valueChanges.pipe(map(result => result.data.partyByType));
  }


  public getFirstKanbanTaskRef() {
    return this.firstKanbanTask.watch().valueChanges.pipe(map(result => result.data.KanbanFirstTask));
  }
  ...
  {
    return this.kanbanUpdateTask.mutate({ task_id: taskId, taskInput: taskData});
  }
